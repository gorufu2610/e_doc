...
<?php
    ...
    if(isset($_REQUEST['delete_id'])){
        ...
        $select_stmt = $db->prepare('SELECT * FROM revice_mk WHERE id = :id');
        $select_stmt->bindParam(':id', $id);
        $select_stmt->execute();
        $row = $select_stmt->fetch(PDO::FETCH_ASSOC);
        unlink("upload/".$row['img']);

        header("Location: revice.php");
...
<body>
    ...
    <li class="nav-item">
        <a class="nav-link" href="revice.php">เอกสารที่ได้รับ</a>
    </li>
</ul>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        // รับรายการแท็บและเนื้อหาเอกสาร
        var tabs = document.querySelectorAll('.nav-link');
        var content = document.querySelectorAll('.tab-pane');
            ...
            <table class="table table-striped table-bordered table-hover">
                ...
                <tbody>
                    <?php                                           //เปลี่ยนของแผนกอื่นตรงนี้
                        $select_stmt = $db->prepare('SELECT * FROM revice_mk');
                        $select_stmt->execute();
                    ...
                    <tr>
                        ...
                        <td><a href="view_revice.php?update_id=<?php echo $row['id']; ?>" class="btn btn-success">ดูเอกสาร</a></td>
                    </tr>
                    <?php } ?>
                </tbody>
            </table>
